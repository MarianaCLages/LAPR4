...
* In the beginning, the system will ask for a questionnaire ID, which later on, will validate if there exists any questionnaire with the given ID.


* Afterwards, if the questionnaire is valid and exists, the system will start computing all statistical information to generate the report. For the given questionnaire, the system will recall all client answers given that questionnaire and finally generate the report.


* In the end, the system will inform the actor that the report was successfully created and show the file path.
...
- Application Layer:
    - GenerateStatisticalReportUController
