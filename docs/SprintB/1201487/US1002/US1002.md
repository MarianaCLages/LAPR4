...
- Demo 1.1 In this specific sprint, we will only focus in the option **a.** as stated previously.
  ...
  + In order to make this demo happen, we first have to analyze what is demanded in this Use Case scenario.
...
Since the requirments of this Use Case was able to see all products with filters, or without, and represent them with a order, or not, we opted to make a service to filter the product list and sort it (considering the options introduced by the Actor), this way we can abstract the Controller, Application Layer, from the Repository Layer, applying this way the best as we can the Grap Pattern.
