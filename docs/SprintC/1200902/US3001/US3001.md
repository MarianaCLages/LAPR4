...
Consult answers of a surveys. It consists of presenting in raw all the answers received regarding a given survey. It must allow:
...
- Demo 1.1: In this User story, we will only focus in the whole use case:
  ...
  + In order to make this possible, we must create a grammar in ANTLR4 that can read all types of VALID QUESTIONNAIRES, if a invalid questionnaire is introduced by the actor, the system must alert the same and specify the error found;
  + Also, we must verify the file that is introduced in the system, if it exists, and in the end of this feature we must persist the content of the survey in our database running in the cloud.
...
In the requested grammar, is it needed to distinguish the different types of questions? For example, if the user says a certain question is multiple choice, shall we make the user input the possible options specifically or do we let the user write the content of the question as he prefers?
...
Well, the question type is need and useful for some reason.
...
* In the beginning, the system will ask certain informations to the actor so the survey can be correctly built. Which are rules (can be multiple and must be alphaNumeric), survey code (that must be also a alphaNumeric), survey description, the survey period of time (which must be a valid number) and the questionnaire path, which later on, the sytem will verify the integrity of the file;
...
This feature can be executed inside the warehouse manager, or understand how the feature will role by seeing all tests developed.
